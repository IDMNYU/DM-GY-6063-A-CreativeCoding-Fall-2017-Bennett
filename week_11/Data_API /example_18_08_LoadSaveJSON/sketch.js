...
function draw() {
  ...
  for (var i = 0; i < bubbles_array.length; i++) {
    bubbles_array[i].display();
    bubbles_array[i].rollover(mouseX, mouseY);
  }
...
function loadData() {
  ...
  for (var i = 0; i < json.bubbles.length; i++) {
    ...
    bubbles_array[i] = new Bubble(bubble.position.x, bubble.position.y, bubble.diameter, bubble.label);
  }
}
