...
boolean cueBirds = false;
boolean madeBirds = false;
Particle[] birdz; 
//int currentBird = 0;
...
void setup() {
  ...
  birdz = new Particle[15];
  /*
  for (int i = 0; i <15; i++) {
   Particle b = new Particle(new PVector(0, random(0, height)));
   }
   */
  interval = 5000;
}
...
void draw() {
  ...
  if (cueBirds == false && madeBirds == true) {
   
    
    for (int i = 0; i < birdz.length; i++) {
      birdz[i].run();
      if (birdz[i].isDead()) { // check to see if we are dead
        // if we are, take us out of the array
        shorten(birdz); // at this indice point, remove one indice
      }
    }
  }
}
